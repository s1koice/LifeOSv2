...
export function OrbitCompass({data,size=320,centerLabel="СЕГОДНЯ",centerValue,highlightId}){
  const cx=size/2,cy=size/2,radius=size*0.355;
  const max=Math.max(1,...data.map(d=>d.value));
  const avg=data.length?Math.round(data.reduce((s,d)=>s+d.value,0)/data.length/max*100):0;
  const val=centerValue==null?avg:centerValue;
  const weakest=data.reduce((a,b)=>b.value<a.value?b:a,data[0]||{value:0});
  const hi=highlightId||weakest?.id;
  const ns=size*0.108;
  return <div style={{position:"relative",width:size,height:size,margin:"0 auto"}}>
    <div style={{position:"absolute",left:size*0.25,top:size*0.25,width:size*0.5,height:size*0.5,borderRadius:"50%",border:`1px solid ${C.border}`,animation:"spin 40s linear infinite"}}/>
    <div style={{position:"absolute",left:size*0.1,top:size*0.1,width:size*0.8,height:size*0.8,borderRadius:"50%",border:`1px solid ${C.border}`,animation:"spinRev 70s linear infinite"}}/>
    <div style={{position:"absolute",left:0,top:0,width:size,height:size,borderRadius:"50%",border:`1px solid ${C.border}`,animation:"spin 100s linear infinite"}}/>
    <div style={{position:"absolute",left:cx-size*0.15,top:cy-size*0.15,width:size*0.3,height:size*0.3,borderRadius:"50%",background:`radial-gradient(circle at 35% 30%, ${C.card2}, ${C.bg} 72%)`,border:`1px solid ${C.border}`,boxShadow:`0 0 ${size*0.17}px var(--hi)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,zIndex:3}}>
      <b style={{fontSize:size*0.031,letterSpacing:1,color:C.muted,fontWeight:500}}>{centerLabel}</b>
      <span style={{fontSize:size*0.082,color:C.text,fontWeight:600,letterSpacing:-0.5}}>{val}%</span>
    </div>
    {data.map((d,i)=>{
      ...
      return <div key={d.id||i}>
        <div style={{position:"absolute",left:x-ns/2,top:y-ns/2,width:ns,height:ns,borderRadius:"50%",background:isHi?"linear-gradient(135deg,var(--amber),#e0863f)":C.card2,border:isHi?"none":`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:ns*0.44,boxShadow:isHi?`0 0 ${size*0.075}px color-mix(in srgb, var(--amber) 55%, transparent)`:"0 4px 14px rgba(0,0,0,.4)",zIndex:2}}>{d.icon}</div>
        <div style={{position:"absolute",top:y-size*0.022,[right?"left":"right"]:right?x+ns*0.72:size-x+ns*0.72,fontSize:size*0.033,color:C.muted,whiteSpace:"nowrap"}}>{d.area}</div>
      </div>})}
  </div>;
}
