...
function AgenticCommandCenter({brief,busy,suggestions,memory,actionLog,links,routineRuns,navigate,onRunSkill,onAccept,onDismiss,onAsk,onEvaluateMemory}:{brief:AgenticBrief;busy:boolean;suggestions:AgentSuggestion[];memory:AgentMemory[];actionLog:AgentActionLog[];links:EntityLink[];routineRuns:RoutineRun[];navigate:(section:Section)=>void;onRunSkill:(skill:AgenticSkill)=>void;onAccept:(id:string)=>void;onDismiss:(id:string)=>void;onAsk:(id:string)=>void;onEvaluateMemory:(id:string,status:"saved"|"discarded")=>void}){
  ...
  return <section className="agentic-command-center" aria-label="Умный центр дня">
    ...
    <details className="agentic-disclosure"><summary><span><strong>Ещё</strong><small>Навыки, память и история AI</small></span><b>Открыть</b></summary><div className="agentic-disclosure-body">
...
function CommandMenu({tasks,projects,goals,onClose,onNavigate,onCreate,onOpenTask}:{tasks:Task[];projects:Project[];goals:Goal[];onClose:()=>void;onNavigate:(section:Section)=>void;onCreate:(kind:Exclude<ModalKind,null>)=>void;onOpenTask:(task:Task)=>void}){
  ...
  function keyDown(event:React.KeyboardEvent<HTMLInputElement>){if(event.key==="ArrowDown"){event.preventDefault();setActiveIndex(index=>actions.length?(index+1)%actions.length:0)}else if(event.key==="ArrowUp"){event.preventDefault();setActiveIndex(index=>actions.length?(index-1+actions.length)%actions.length:0)}else if(event.key==="Enter"&&actions[activeIndex]){event.preventDefault();actions[activeIndex].run()}else if(event.key==="Escape")onClose()}
  return <div className="command-wrap" onMouseDown={onClose}><section className="command-menu" onMouseDown={event=>event.stopPropagation()}><div className="command-search"><span>⌕</span><input autoFocus value={query} onChange={event=>setQuery(event.target.value)} onKeyDown={keyDown} placeholder="Найти задачу, цель или выполнить команду…"/><kbd>esc</kbd></div><div className="command-results"><small>{normalized?"РЕЗУЛЬТАТЫ":"БЫСТРЫЕ ДЕЙСТВИЯ"}</small>{actions.map((action,index)=><button type="button" className={activeIndex===index?"active":""} key={`${action.title}-${index}`} onMouseEnter={()=>setActiveIndex(index)} onClick={action.run}><span>{action.icon}</span><div><strong>{action.title}</strong><small>{action.hint}</small></div>{activeIndex===index&&<kbd>↵</kbd>}</button>)}{!actions.length&&<p>Ничего не найдено. Нажмите «Новая задача», чтобы сохранить мысль.</p>}</div><footer><span>↑↓ выбрать</span><span>↵ открыть</span><span>⌘K в любом разделе</span></footer></section></div>;
}
