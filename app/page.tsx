...
type UndoSnapshot = { id: number; label: string; createdAt: string; tasks: Task[]; projects: Project[]; goals: Goal[]; habits: Habit[]; events: CalendarEvent[]; inboxItems: InboxItem[]; resources: ResourceItem[] };
...
export default function Home() {
  ...
  const [profile, setProfile] = useState<UserProfile>({ name: "Алексей", textScale: "large", dailyCapacityMinutes: 360 });
  ...
  const applyStoredState = useCallback((parsed: Record<string, unknown>) => {
    ...
    if (parsed.profile && typeof parsed.profile === "object") {
      ...
      setProfile({ name: typeof next.name === "string" && next.name.trim() ? next.name.trim() : "Алексей", textScale: next.textScale === "normal" || next.textScale === "extra" ? next.textScale : "large", dailyCapacityMinutes: [240,360,480].includes(Number(next.dailyCapacityMinutes)) ? Number(next.dailyCapacityMinutes) : 360 });
    }
  ...
  function captureUndo(label: string) {
    setUndoStack(current => [{ id: newEntityId(), label, createdAt: new Date().toISOString(), tasks, projects, goals, habits, events, inboxItems, resources }, ...current].slice(0, 10));
  }
...
function FinancePage({accounts,setAccounts,transactions,setTransactions,categories,setCategories,budgetLines,setBudgetLines,recurringExpenses,setRecurringExpenses,loans,setLoans,byok,onRenameCategory,onDeleteCategory,onOperation,onTransfer,onAccount}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];setCategories:React.Dispatch<React.SetStateAction<FinanceCategory[]>>;budgetLines:BudgetLine[];setBudgetLines:React.Dispatch<React.SetStateAction<BudgetLine[]>>;recurringExpenses:RecurringExpense[];setRecurringExpenses:React.Dispatch<React.SetStateAction<RecurringExpense[]>>;loans:Loan[];setLoans:React.Dispatch<React.SetStateAction<Loan[]>>;byok:string;onRenameCategory:(previousName:string,nextName:string)=>void;onDeleteCategory:(name:string)=>void;onOperation:()=>void;onTransfer:()=>void;onAccount:()=>void}){
  ...
  return <>
    <PageTitle eyebrow="РЕСУРСЫ" title="Финансы" text="План, подтверждённый факт и движение денег без двойного учёта." action="Операция" onAction={onOperation} extra={<><button type="button" className="secondary-action import-action" onClick={()=>setImportOpen(true)}>✦ AI‑импорт</button><button type="button" className="secondary-action" onClick={()=>setRealBalanceOpen(true)}>◎ Реальный баланс</button><button type="button" className="secondary-action" onClick={()=>setBudgetOpen(true)}>Бюджет</button><button type="button" className="secondary-action" onClick={()=>setCategoriesOpen(true)}>Категории</button><button type="button" className="secondary-action" onClick={onTransfer}>Перевод</button><button type="button" className="secondary-action" onClick={onAccount}>Счёт</button></>}/>
    ...
    {importOpen&&<BankImportModal accounts={accounts} setAccounts={setAccounts} transactions={transactions} setTransactions={setTransactions} categories={categories} byok={byok} onClose={()=>setImportOpen(false)}/>}
...
function BankImportModal({accounts,setAccounts,transactions,setTransactions,categories,byok,onClose}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];byok:string;onClose:()=>void}){
  const[records,setRecords]=useState<BankImportRecord[]>([]);const[selected,setSelected]=useState<Set<string>>(new Set());const[fileSummary,setFileSummary]=useState<{name:string;format:string;found:number;ignored:number}[]>([]);const[ignored,setIgnored]=useState(0);const[batchDuplicates,setBatchDuplicates]=useState(0);const[loading,setLoading]=useState(false);const[error,setError]=useState("");const[imported,setImported]=useState<number|null>(null);const[aiNote,setAiNote]=useState("");
  ...
  async function analyzeImages(files:File[]){
    const form=new FormData();files.forEach(file=>form.append("files",file));form.append("context",JSON.stringify({categories:categories.map(item=>item.name),accounts:accounts.map(item=>({name:item.name,type:item.type,last4:item.last4})),recentTransactions:transactions.slice(0,250).map(item=>({date:item.date,title:item.title,amount:item.amount,kind:item.kind,account:accounts.find(account=>account.id===item.accountId)?.name,merchantOriginal:item.merchantOriginal}))}));
  ...
  function commit(){
    const chosen=records.filter(record=>selected.has(record.id)&&!isExisting(record));if(!chosen.length){setImported(0);return}
    ...
    const created=createImportedTransactions(chosen,resolveAccount);const seriesIds=new Set(chosen.map(record=>record.installmentSeriesId).filter(Boolean));const normalized=normalizePersonalAccounts(nextAccounts,created);setAccounts(normalized.accounts);setTransactions(current=>[...normalized.entries,...current.filter(item=>!(item.posted===false&&item.installmentSeriesId&&seriesIds.has(item.installmentSeriesId)))]);setImported(created.length);setSelected(new Set());
  }
