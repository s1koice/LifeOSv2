...
type Task = { id: number; title: string; area: string; time: string; done: boolean; priority: Priority; projectId?: number; dueDate?: string; archived?: boolean; duration?: number; energy?: TaskEnergy; context?: TaskContext; createdAt?: string; completedAt?: string };
...
type Goal = { id: number; period: string; date: string; title: string; note: string; progress: number; area?: string; projectIds?: number[] };
...
const seedTasks: Task[] = [
  { id: 1, title: "Завершить структуру лендинга", area: "Карьера", time: "10:00", done: false, priority: "high", projectId: 1, duration: 90, energy: "high", context: "computer" },
...
const seedGoals: Goal[] = [
  ...
  { id: 2, period: "КВАРТАЛ", date: "Q3", title: "Запустить и внедрить NEXUS OS", note: "Единая система вместо разрозненных инструментов.", progress: 61, projectIds: [1] },
  { id: 3, period: "МЕСЯЦ", date: "Август", title: "Собрать рабочий MVP", note: "Dashboard, планирование и ежедневные ритуалы.", progress: 68, projectIds: [1] },
  { id: 4, period: "НЕДЕЛЯ", date: "Эта неделя", title: "Закрыть ключевые экраны", note: "Пять измеримых результатов.", progress: 60, projectIds: [1, 3] },
  { id: 5, period: "СЕГОДНЯ", date: "Сегодня", title: "Закончить основу", note: "2 часа 30 минут глубокого фокуса.", progress: 32, projectIds: [1] },
];
...
function goalProgressValue(goal: Goal, projects: Project[], tasks: Task[]) {
...
export default function Home() {
  ...
  const applyStoredState = useCallback((parsed: Record<string, unknown>) => {
    ...
    if (Array.isArray(parsed.goals)) setGoals(uniqueEntityIds(parsed.goals as Goal[]));
  ...
  }, [loaded, tasks, habits, projects, goals, inboxItems, resources, events, journalEntries, healthNotes, weeklyReviews, transactions, planningFocuses]);
  ...
  function chooseQuickAdd(kind: Exclude<ModalKind, null>) { setQuickAddOpen(false); setEditingHabit(null); setTaskProjectId(null); setModalKind(kind); }
  ...
  function handleCreate(kind: Exclude<ModalKind, null>, draft: Record<string, unknown>) {
    const id = newEntityId();
    if (kind === "task") { const projectId = Number(draft.projectId || taskProjectId || 0) || undefined; const linkedProject = projects.find(project => project.id === projectId); setTasks(v => [...v, { id, title: String(draft.title), area: linkedProject?.area || String(draft.area || "Личное"), time: String(draft.time || "Сегодня"), dueDate: String(draft.dueDate || "") || undefined, priority: (draft.priority || "medium") as Priority, projectId, done: false, duration: Math.max(5, Number(draft.duration || 30)), energy: (draft.energy || "medium") as TaskEnergy, context: (draft.context || "anywhere") as TaskContext, createdAt: new Date().toISOString() }]); }
  ...
  function content() {
    if (section === "Обзор") return <Dashboard tasks={tasks} habits={habits} projects={projects} goals={goals} lifeAreas={lifeAreas} events={events} transactions={transactions} budgetLines={budgetLines} weeklyReviews={weeklyReviews} taskCompletion={taskCompletion} habitCompletion={habitCompletion} today={clock.today} onToggleTask={toggleTask} onToggleHabit={toggleHabit} navigate={navigate} order={dashboardOrder} setOrder={setDashboardOrder} gamification={gamification} onGameCheckIn={gameCheckIn} onPlanWithAi={()=>{setPrompt("Спланируй сегодняшний день: выбери реалистичные приоритеты, назначь время с учётом длительности, энергии и контекста и сразу внеси изменения");setAssistantOpen(true)}}/>;
  ...
  return <div className={`app-shell theme-${theme}`}>
    ...
    {authChecked&&authOpen&&<AuthPanel session={authSession} syncStatus={syncStatus} syncError={syncError} lastSyncedAt={lastSyncedAt} onSession={acceptAuthSession} onSignOut={handleSignOut} onSync={()=>void syncNow()} onBackup={downloadBackup} onRestore={file=>void restoreBackup(file)} onClose={()=>{if(authSession)setAuthOpen(false)}}/>} 
...
function Dashboard({tasks,habits,projects,goals,lifeAreas,events,transactions,budgetLines,weeklyReviews,taskCompletion,habitCompletion,today,onToggleTask,onToggleHabit,navigate,order,setOrder,gamification,onGameCheckIn,onPlanWithAi}:{tasks:Task[];habits:Habit[];projects:Project[];goals:Goal[];lifeAreas:LifeArea[];events:CalendarEvent[];transactions:Transaction[];budgetLines:BudgetLine[];weeklyReviews:ParaWeeklyReview[];taskCompletion:number;habitCompletion:number;today:string;onToggleTask:(id:number)=>void;onToggleHabit:(id:number,date:string)=>void;navigate:(s:Section)=>void;order:DashboardBlockId[];setOrder:React.Dispatch<React.SetStateAction<DashboardBlockId[]>>;gamification:GamificationState;onGameCheckIn:(kind:"setback"|"recovery")=>void;onPlanWithAi:()=>void}) {
  ...
  const blocks:Record<DashboardBlockId,React.ReactNode>={
    dayCenter:<section className="card smart-day-center"><div className="smart-day-head"><div><span className="eyebrow">УМНЫЙ ЦЕНТР ДНЯ</span><h2>{candidates.length?`Сегодня реально закрыть ${candidates.length} важных действия`:"Главные действия на сегодня закрыты"}</h2><p>{taskMinutes} минут фокуса · {calendarToday.length} событий · {habitsLeft} привычек осталось{overdueCount?` · ${overdueCount} просрочено`:""}</p></div><button type="button" className="primary" onClick={onPlanWithAi}>✦ Спланировать с AI</button></div><div className="smart-day-grid"><div className="smart-priorities"><strong>РЕКОМЕНДУЕМЫЙ ПОРЯДОК</strong>{candidates.map((task,index)=><label className={`${task.energy==="high"?"high-energy":""} ${task.dueDate&&task.dueDate<today?"overdue":""}`} key={task.id}><button type="button" onClick={()=>onToggleTask(task.id)} aria-label={`Выполнить ${task.title}`}>○</button><span><small>{task.dueDate&&task.dueDate<today?"ПРОСРОЧЕНО":`0${index+1} · ${task.time||"Без времени"}`}</small><b>{task.title}</b><em>{task.duration||30} мин · {contextLabel[task.context||"anywhere"]}</em></span></label>)}{!candidates.length&&<p className="empty-copy">Можно посвятить время восстановлению или следующему проекту.</p>}</div><div className="smart-day-signals"><article><span>◷</span><div><small>БЛИЖАЙШЕЕ В КАЛЕНДАРЕ</small><strong>{calendarToday[0]?.title||"Свободное окно"}</strong><em>{calendarToday[0]?.time||"День без встреч"}</em></div></article><article><span>₪</span><div><small>БЕЗОПАСНЫЙ ТЕМП</small><strong>{formatIls(safeToday)} сегодня</strong><em>{formatIls(Math.max(0,monthBudget-monthSpent))} до конца месяца</em></div></article><button type="button" className={reviewDone?"done":""} onClick={()=>navigate("Проекты")}><span>{reviewDone?"✓":"◎"}</span><div><small>ОБЗОР PARA</small><strong>{reviewDone?"Неделя разобрана":"Пора проверить систему"}</strong><em>{reviewDone?"Следующий обзор в воскресенье":"Пошаговый обзор займёт 10 минут"}</em></div></button></div></div></section>,
...
function TasksPage({tasks,projects,setTasks,onNew}:{tasks:Task[];projects:Project[];setTasks:React.Dispatch<React.SetStateAction<Task[]>>;onNew:()=>void}){
  const[filter,setFilter]=useState<"active"|"done"|"all">("active");const[context,setContext]=useState<TaskContext|"all">("all");const visible=tasks.filter(t=>(filter==="all"||(filter==="done"?t.done:!t.done))&&(context==="all"||(t.context||"anywhere")===context));
  return <><PageTitle eyebrow="ДЕЙСТВИЕ" title="Задачи" text="У каждой задачи есть оценка времени, требуемая энергия и контекст — проще выбрать действие под реальную ситуацию." action="Новая задача" onAction={onNew}/><div className="stats-strip"><Stat value={tasks.filter(t=>!t.done).length.toString()} label="В работе"/><Stat value={tasks.filter(t=>t.done).length.toString()} label="Выполнено"/><Stat value={`${tasks.filter(t=>!t.done).reduce((sum,t)=>sum+(t.duration||30),0)} мин`} label="Оценка остатка"/><Stat value={`${tasks.length?Math.round(tasks.filter(t=>t.done).length/tasks.length*100):0}%`} label="Темп"/></div><section className="card full-card"><div className="filter-row"><button type="button" className={filter==="active"?"active":""} onClick={()=>setFilter("active")}>В работе</button><button type="button" className={filter==="done"?"active":""} onClick={()=>setFilter("done")}>Выполнено</button><button type="button" className={filter==="all"?"active":""} onClick={()=>setFilter("all")}>Все</button><select aria-label="Фильтр по контексту" value={context} onChange={event=>setContext(event.target.value as TaskContext|"all")}><option value="all">Все контексты</option>{Object.entries(contextLabel).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select><span/><button type="button" onClick={onNew}>＋ Добавить</button></div><div className="large-task-list">{visible.length?visible.map(t=>{const project=projects.find(p=>p.id===t.projectId);return <label key={t.id} className={t.done?"done":""}><input type="checkbox" checked={t.done} onChange={()=>setTasks(v=>v.map(x=>x.id===t.id?{...x,done:!x.done,completedAt:x.done?undefined:new Date().toISOString()}:x))}/><span className={`check ${t.priority}`}>✓</span><div><strong>{t.title}</strong><small><b>{t.area}</b>{project&&<em className="project-link">▦ {project.name}</em>} · {t.dueDate?prettyDate(t.dueDate):t.time}</small><span className="task-attributes"><em>◷ {t.duration||30} мин</em><em className={`energy-${t.energy||"medium"}`}>⚡ {energyLabel[t.energy||"medium"]}</em><em>⌁ {contextLabel[t.context||"anywhere"]}</em></span></div><button type="button" aria-label="Удалить задачу" onClick={e=>{e.preventDefault();setTasks(v=>v.filter(x=>x.id!==t.id))}}>×</button></label>}):<EmptyState text="В этом списке пока ничего нет" action="Создать задачу" onClick={onNew}/>}</div></section></>;
}
...
function GoalsPage({goals,setGoals,projects,tasks,onNew}:{goals:Goal[];setGoals:React.Dispatch<React.SetStateAction<Goal[]>>;projects:Project[];tasks:Task[];onNew:()=>void}){
  ...
  return <><PageTitle eyebrow="НАПРАВЛЕНИЕ" title="Цели и проекты" text="Свяжите цель с проектами — прогресс будет считаться автоматически по выполненным задачам и этапам." action="Новая цель" onAction={onNew}/><div className="stats-strip"><Stat value={`${average}%`} label="Общий прогресс"/><Stat value={goals.filter(goal=>(goal.projectIds?.length||0)>0).length.toString()} label="Целей связаны"/><Stat value={projects.filter(project=>!project.archived).length.toString()} label="Активных проектов"/><Stat value={tasks.filter(task=>task.done).length.toString()} label="Задач завершено"/></div><section className="timeline-chain goal-timeline">{goals.map((goal,i)=>{const progress=goalProgressValue(goal,projects,tasks);const linked=projects.filter(project=>goal.projectIds?.includes(project.id));return <div className={`${i===2?"accent":""} ${selectedGoalId===goal.id?"selected":""}`} key={goal.id}><em>{goal.period}</em><span>{goal.date}</span><h3>{goal.title}</h3><p>{goal.note}</p><div className="goal-project-chips">{linked.slice(0,2).map(project=><small key={project.id}>▦ {project.name}</small>)}{linked.length>2&&<small>＋{linked.length-2}</small>}</div><div className="entity-actions"><button type="button" onClick={()=>setSelectedGoalId(selectedGoalId===goal.id?null:goal.id)}>Связать проекты</button>{!linked.length&&<button type="button" onClick={()=>setGoals(v=>v.map(row=>row.id===goal.id?{...row,progress:Math.min(100,row.progress+10)}:row))}>＋10%</button>}<button type="button" onClick={()=>{setGoals(v=>v.filter(row=>row.id!==goal.id));setSelectedGoalId(null)}}>Удалить</button></div><b>{progress}%</b><small className="progress-source">{linked.length?"автоматически":"вручную"}</small></div>})}</section>{selected&&<section className="card goal-link-panel"><div><span className="eyebrow">СВЯЗЬ ЦЕЛИ С РЕЗУЛЬТАТАМИ</span><h3>{selected.title}</h3><p>Выберите проекты, которые двигают эту цель. Их прогресс сразу изменит процент цели.</p></div><div>{projects.filter(project=>!project.archived).map(project=>{const checked=!!selected.projectIds?.includes(project.id);const progress=projectProgressValue(project,tasks);return <label className={checked?"selected":""} key={project.id}><input type="checkbox" checked={checked} onChange={()=>toggleProject(selected.id,project.id)}/><span>{checked?"✓":""}</span><div><strong>{project.name}</strong><small>{project.area} · {progress}%</small></div><div className="mini-track"><i style={{width:`${progress}%`}}/></div></label>})}</div><button type="button" className="primary" onClick={()=>setSelectedGoalId(null)}>Готово</button></section>}</>;
}
...
function AuthPanel({session,syncStatus,syncError,lastSyncedAt,onSession,onSignOut,onSync,onBackup,onRestore,onClose}:{session:PinSession|null;syncStatus:SyncStatus;syncError:string;lastSyncedAt:string;onSession:(session:PinSession)=>void;onSignOut:()=>void;onSync:()=>void;onBackup:()=>void;onRestore:(file:File)=>void;onClose:()=>void}){
...
function CreateModal({kind,accounts,projects,areas,categories,initialProjectId,initialHabit,onClose,onCreate}:{kind:Exclude<ModalKind,null>;accounts:Account[];projects:Project[];areas:LifeArea[];categories:FinanceCategory[];initialProjectId:number|null;initialHabit:Habit|null;onClose:()=>void;onCreate:(draft:Record<string,unknown>)=>void}){
  ...
  return <div className="modal-wrap" onMouseDown={onClose}><form className="quick-modal entity-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}><div><span className="eyebrow">NEXUS · СОЗДАНИЕ</span><button type="button" onClick={onClose}>×</button></div><h2>{titles[kind]}</h2>
    ...
    {kind==="task"&&<div className="modal-grid"><SelectField label="Проект · необязательно" name="projectId" options={[["","Без проекта"],...projects.filter(p=>!p.archived).map(p=>[String(p.id),p.name])]} defaultValue={String(initialProjectId||"")}/><Field label="Дата" name="dueDate" type="date" placeholder="" required={false}/><Field label="Время" name="time" type="time" placeholder="Сегодня" required={false}/><SelectField label="Приоритет" name="priority" options={[["medium","Средний"],["high","Высокий"],["low","Низкий"]]}/><SelectField label="Продолжительность" name="duration" options={[["15","15 минут"],["30","30 минут"],["45","45 минут"],["60","1 час"],["90","1,5 часа"],["120","2 часа"]]} defaultValue="30"/><SelectField label="Энергия" name="energy" options={[["medium","Средняя"],["high","Высокая"],["low","Низкая"]]} defaultValue="medium"/><SelectField label="Контекст" name="context" options={[["anywhere","Где угодно"],["computer","За компьютером"],["phone","Телефон"],["home","Дома"],["outside","Вне дома"]]} defaultValue="anywhere"/></div>}
