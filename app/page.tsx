...
type Account = { id: number; name: string; type: AccountType; balance: number; linkedAccountId?: number; billingDay?: number; last4?: string };
...
export default function Home() {
  ...
  function chooseQuickAdd(kind: Exclude<ModalKind, null>) { setQuickAddOpen(false); setEditingHabit(null); setEditingTask(null); setEditingGoal(null); setTaskProjectId(null); setModalKind(kind); }
  ...
  function handleCreate(kind: Exclude<ModalKind, null>, draft: Record<string, unknown>) {
    ...
    notify(editingHabit ? "Привычка обновлена" : editingTask ? "Задача обновлена" : editingGoal ? "Цель обновлена" : "Сохранено в NEXUS OS"); setModalKind(null); setEditingHabit(null); setEditingTask(null); setEditingGoal(null); setTaskProjectId(null);
  }
  ...
  return <div className={`app-shell theme-${theme} text-${profile.textScale}`}>
    ...
    {modalKind && <CreateModal kind={modalKind} accounts={accounts} projects={projects} goals={goals} areas={lifeAreas} categories={financeCategories} setCategories={setFinanceCategories} onRenameCategory={renameFinanceCategory} onDeleteCategory={deleteFinanceCategory} initialProjectId={taskProjectId} initialHabit={editingHabit} initialTask={editingTask} initialGoal={editingGoal} onClose={() => { setModalKind(null); setEditingHabit(null); setEditingTask(null); setEditingGoal(null); setTaskProjectId(null); }} onCreate={draft => handleCreate(modalKind, draft)}/>} 
...
function FinancePage({accounts,setAccounts,transactions,setTransactions,categories,setCategories,budgetLines,setBudgetLines,recurringExpenses,setRecurringExpenses,loans,setLoans,onRenameCategory,onDeleteCategory,onOperation,onTransfer,onAccount}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];setCategories:React.Dispatch<React.SetStateAction<FinanceCategory[]>>;budgetLines:BudgetLine[];setBudgetLines:React.Dispatch<React.SetStateAction<BudgetLine[]>>;recurringExpenses:RecurringExpense[];setRecurringExpenses:React.Dispatch<React.SetStateAction<RecurringExpense[]>>;loans:Loan[];setLoans:React.Dispatch<React.SetStateAction<Loan[]>>;onRenameCategory:(previousName:string,nextName:string)=>void;onDeleteCategory:(name:string)=>void;onOperation:()=>void;onTransfer:()=>void;onAccount:()=>void}){
  const[categoriesOpen,setCategoriesOpen]=useState(false);const[budgetOpen,setBudgetOpen]=useState(false);const[recurringOpen,setRecurringOpen]=useState(false);const[importOpen,setImportOpen]=useState(false);const[editingRecurring,setEditingRecurring]=useState<RecurringExpense|null>(null);const[splitting,setSplitting]=useState<Transaction|null>(null);const[selectedAccountId,setSelectedAccountId]=useState<number|null>(null);
  ...
  const selectedAccount=accounts.find(account=>account.id===selectedAccountId);const matchesSelectedAccount=(item:Transaction)=>selectedAccountId===null||item.accountId===selectedAccountId||item.toAccountId===selectedAccountId;const journalTransactions=transactions.filter(item=>matchesSelectedAccount(item)&&(item.kind!=="installment"||item.posted)).sort((a,b)=>b.date.localeCompare(a.date)||b.id-a.id);
  ...
  <FinanceForecast accounts={accounts} transactions={transactions} budgetLines={budgetLines} recurringExpenses={recurringExpenses} loans={loans}/>
  <FinanceReconciliation transactions={transactions} setTransactions={setTransactions} categories={categories} onImport={()=>setImportOpen(true)}/>
  ...
  <FinanceCalendar recurringExpenses={recurringExpenses} transactions={transactions} accounts={accounts} loans={loans}/>
  <LoansSection loans={loans} setLoans={setLoans}/>
  <section className="wallet-section"><div className="wallet-section-head"><div><span className="eyebrow">КОШЕЛЬКИ И СЧЕТА</span><h3>Расходы по источникам</h3><p>Нажмите на карточку, чтобы оставить в журнале только операции этого счёта.</p></div><button type="button" className={selectedAccountId===null?"active":""} onClick={()=>setSelectedAccountId(null)}>Все счета</button></div><div className="accounts-grid">{accounts.map(account=>{const accountSpent=monthTransactions.filter(item=>item.kind==="expense"&&item.accountId===account.id).reduce((sum,item)=>sum+item.amount,0);const transactionCount=transactions.filter(item=>item.accountId===account.id||item.toAccountId===account.id).length;return <AccountCard key={account.id} account={account} monthlySpent={accountSpent} selected={selectedAccountId===account.id} transactionCount={transactionCount} onSelect={()=>setSelectedAccountId(current=>current===account.id?null:account.id)} onSave={next=>setAccounts(v=>v.map(a=>a.id===next.id?next:a))} onDelete={()=>{setAccounts(current=>current.filter(item=>item.id!==account.id));setSelectedAccountId(current=>current===account.id?null:current)}}/>})}</div></section>
  <div className="finance-grid"><section className="card budget budget-vs-actual"><CardHead title="Бюджет: план и факт" subtitle={`${new Intl.DateTimeFormat("ru-RU",{month:"long"}).format(now)} · ${budgetUsage}%`} action="Настроить" onClick={()=>setBudgetOpen(true)}/>{categories.map(category=>{const actual=monthTransactions.filter(t=>t.kind==="expense"&&t.category===category.name).reduce((sum,t)=>sum+t.amount,0);const plan=budgetLines.find(line=>line.category===category.name)?.limit||0;const percent=plan?Math.round(actual/plan*100):actual?100:0;return <div className={actual>plan&&plan>0?"over":""} key={category.id}><span>{category.icon} {category.name}</span><div><div className="mini-track"><i style={{width:`${Math.min(100,percent)}%`}}/></div><small>{percent}%</small></div><b>{formatIls(actual)} <small>/ {formatIls(plan)}</small></b></div>})}</section><section className="card installments"><CardHead title="План кредитных платежей" subtitle={`${upcoming.length} будущих платежей`}/>{upcoming.length?upcoming.slice(0,8).map(item=><div className="installment-row" key={item.id}><span>{prettyDate(item.dueDate||item.date)}</span><div><strong>{item.title}</strong><small>план платежа · расход уже учтён в покупке</small></div><b>{formatIls(item.amount)}</b></div>):<p className="empty-copy">Добавьте покупку с кредитной карты и выберите число платежей.</p>}</section></div>
  ...
  <section className="card transactions"><CardHead title={selectedAccount?`Журнал · ${selectedAccount.name}`:"Журнал операций"} subtitle={`${journalTransactions.length} записей · новые сверху`} action="Новая операция" onClick={onOperation}/>{selectedAccount&&<div className="journal-account-filter"><span>{selectedAccount.type==="cash"?"▣":selectedAccount.type==="debit"?"▤":"◇"} Показаны операции: <strong>{selectedAccount.name}</strong></span><button type="button" onClick={()=>setSelectedAccountId(null)}>Показать все счета ×</button></div>}{journalTransactions.map(t=><div className="transaction-row" key={t.id}><span className={t.kind}>{t.kind==="income"?"↑":t.kind==="transfer"?"⇄":t.recurringId?"↻":"↓"}</span><div><strong>{t.title}</strong><small>{categories.find(c=>c.name===t.category)?.icon} {t.category} · {prettyDate(t.date)} · {accounts.find(a=>a.id===t.accountId)?.name}{t.recurringId?" · постоянный платёж":""}{t.parentId?" · разделено":""}</small></div><b className={t.kind}>{t.kind==="income"?"+":t.kind==="transfer"?"":"−"}{formatIls(t.amount)}</b><div className="transaction-actions">{t.kind==="expense"&&t.posted!==false&&t.amount>.01&&<button type="button" className="transaction-split" onClick={()=>setSplitting(t)}>Разделить</button>}<button type="button" className="transaction-delete" aria-label={`Удалить ${t.title}`} onClick={()=>setTransactions(v=>v.filter(x=>x.id!==t.id))}>×</button></div></div>)}</section>
  {splitting&&<SplitTransactionModal transaction={splitting} categories={categories} onClose={()=>setSplitting(null)} onSplit={(first,second)=>{setTransactions(current=>current.flatMap(item=>item.id===splitting.id?[first,second]:[item]));setSplitting(null)}}/>} {importOpen&&<BankImportModal accounts={accounts} setAccounts={setAccounts} transactions={transactions} setTransactions={setTransactions} categories={categories} onClose={()=>setImportOpen(false)}/>} {categoriesOpen&&<FinanceCategoriesModal categories={categories} setCategories={setCategories} onRename={onRenameCategory} onDelete={onDeleteCategory} onClose={()=>setCategoriesOpen(false)}/>} {budgetOpen&&<BudgetModal categories={categories} lines={budgetLines} setLines={setBudgetLines} onClose={()=>setBudgetOpen(false)}/>} {recurringOpen&&<RecurringExpenseModal accounts={accounts} categories={categories} initial={editingRecurring} onClose={()=>{setRecurringOpen(false);setEditingRecurring(null)}} onSave={saveRecurring}/>}</>;
}
...
function FinanceCalendar({recurringExpenses,transactions,accounts,loans}:{recurringExpenses:RecurringExpense[];transactions:Transaction[];accounts:Account[];loans:Loan[]}){
  type FinanceCalendarItem={id:string;date:string;title:string;amount:number;kind:"recurring"|"credit"|"loan";paid:boolean;account:string};
  ...
  const recurringItems:FinanceCalendarItem[]=recurringExpenses.filter(item=>item.active).map(item=>{const date=localDateKey(new Date(year,monthIndex,Math.min(item.dayOfMonth,lastDay),12));const paid=transactions.some(transaction=>transaction.kind==="expense"&&transaction.date.startsWith(period)&&(transaction.recurringId===item.id||(transaction.category===item.category&&Math.abs(transaction.amount-item.amount)<.01)));return{id:`recurring-${item.id}`,date,title:item.title,amount:item.amount,kind:"recurring",paid,account:accounts.find(account=>account.id===item.accountId)?.name||"Счёт"}});
  ...
  const loanItems:FinanceCalendarItem[]=loans.filter(item=>item.active&&item.balance>0).map(item=>({id:`loan-${item.id}`,date:localDateKey(new Date(year,monthIndex,Math.min(item.paymentDay,lastDay),12)),title:item.name,amount:item.monthlyPayment,kind:"loan",paid:false,account:"Основной счёт"}));
  const items=[...recurringItems,...creditItems,...loanItems].sort((a,b)=>a.date.localeCompare(b.date));const recurringTotal=recurringItems.reduce((sum,item)=>sum+item.amount,0);const creditTotal=creditItems.reduce((sum,item)=>sum+item.amount,0);const loanTotal=loanItems.reduce((sum,item)=>sum+item.amount,0);
  ...
  return <section className="card finance-calendar"><div className="finance-calendar-head"><div><span className="eyebrow">ДЕНЕЖНЫЙ КАЛЕНДАРЬ</span><h3>Плановые расходы и платежи</h3><p>Календарь напоминает о датах, но ничего не списывает автоматически. Факт появляется из импортированной выписки.</p></div><div className="finance-calendar-summary"><span><small>ПОСТОЯННЫЕ</small><strong>{formatIls(recurringTotal)}</strong></span><span><small>ПЛАТЕЖИ КАРТ</small><strong>{formatIls(creditTotal)}</strong></span><span><small>КРЕДИТЫ</small><strong>{formatIls(loanTotal)}</strong></span><span><small>ВСЕГО ПО ПЛАНУ</small><strong>{formatIls(recurringTotal+creditTotal+loanTotal)}</strong></span></div></div><div className="finance-calendar-toolbar"><button type="button" onClick={()=>move(-1)} aria-label="Предыдущий месяц">←</button><strong>{new Intl.DateTimeFormat("ru-RU",{month:"long",year:"numeric"}).format(month)}</strong><button type="button" onClick={()=>move(1)} aria-label="Следующий месяц">→</button><button type="button" onClick={()=>{const today=new Date();setMonth(new Date(today.getFullYear(),today.getMonth(),1,12))}}>Сегодня</button><div className="finance-calendar-legend"><span>● Постоянный</span><span>● Платёж карты</span><span>● Кредит</span><span>✓ Найден в выписке</span></div></div><div className="finance-calendar-scroll"><div className="finance-calendar-inner"><div className="finance-calendar-weekdays">{dayNames.map(day=><span key={day}>{day}</span>)}</div><div className="finance-calendar-grid">{dates.map(date=>{const key=localDateKey(date);const dayItems=items.filter(item=>item.date===key);return <div className={`finance-calendar-day ${date.getMonth()!==monthIndex?"outside":""} ${key===localDateKey(new Date())?"today":""}`} key={key}><strong>{date.getDate()}</strong><div>{dayItems.map(item=><article className={`finance-calendar-event ${item.kind} ${item.paid?"paid":""}`} title={`${item.title} · ${item.account}`} key={item.id}><span>{item.kind==="credit"?"◇":item.kind==="loan"?"▤":"↻"} {item.title}</span><b>{formatIls(item.amount)}</b><small>{item.paid?"✓ найдено в выписке":item.account}</small></article>)}</div></div>})}</div></div></div></section>;
}

function LoansSection({loans,setLoans}:{loans:Loan[];setLoans:React.Dispatch<React.SetStateAction<Loan[]>>}){
  const totalBalance=loans.filter(item=>item.active).reduce((sum,item)=>sum+item.balance,0);const monthly=loans.filter(item=>item.active).reduce((sum,item)=>sum+item.monthlyPayment,0);
  return <section className="card loans-section"><div className="loans-head"><div><span className="eyebrow">КРЕДИТЫ</span><h3>Остаток и ближайшие платежи</h3><p>Данные добавлены со скриншота банка. Любую сумму, день платежа или прогресс можно изменить.</p></div><div><small>ОБЩИЙ ОСТАТОК</small><strong>{formatIlsExact(totalBalance)}</strong><span>в месяц {formatIlsExact(monthly)}</span></div></div><div className="loans-grid">{loans.map(loan=><LoanCard key={loan.id} loan={loan} onSave={next=>setLoans(current=>current.map(item=>item.id===next.id?next:item))}/>)}</div></section>
}

function LoanCard({loan,onSave}:{loan:Loan;onSave:(loan:Loan)=>void}){
...
function BankImportModal({accounts,setAccounts,transactions,setTransactions,categories,onClose}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];onClose:()=>void}){
  const[records,setRecords]=useState<BankImportRecord[]>([]);const[selected,setSelected]=useState<Set<string>>(new Set());const[fileSummary,setFileSummary]=useState<{name:string;format:string;found:number;ignored:number}[]>([]);const[ignored,setIgnored]=useState(0);const[batchDuplicates,setBatchDuplicates]=useState(0);const[loading,setLoading]=useState(false);const[error,setError]=useState("");const[imported,setImported]=useState<number|null>(null);
  ...
  async function chooseFiles(files:FileList|null){const list=files?Array.from(files):[];if(!list.length)return;setLoading(true);setError("");setImported(null);try{const result=await parseBankFiles(list);const remembered=result.records.map(record=>{const previous=merchantMemory.get(record.originalTitle.normalize("NFKC").toLocaleLowerCase().replace(/\s+/g," ").trim());return previous?{...record,title:previous.title,category:categories.some(category=>category.name===previous.category)?previous.category:record.category}:record});setRecords(remembered);setFileSummary(result.files);setIgnored(result.ignored);setBatchDuplicates(result.duplicatesInFiles);setSelected(new Set(remembered.filter(record=>!isExisting(record)).map(record=>record.id)))}catch(caught){setError(caught instanceof Error?caught.message:"Не удалось прочитать банковские файлы");setRecords([])}finally{setLoading(false)}}
  ...
  function commit(){
    const chosen=records.filter(record=>selected.has(record.id)&&!isExisting(record));
    if(!chosen.length){setImported(0);return}
    const nextAccounts=accounts.map(account=>({...account}));const primaryDebit=nextAccounts.find(account=>account.type==="debit")||nextAccounts[0];
    if(primaryDebit){primaryDebit.name="Основной счёт · Visa •••• 1078";primaryDebit.last4=MAIN_DEBIT_CARD_LAST4}
    const accountIds=new Map<string,number>();
    function resolveAccount(record:BankImportRecord){if(record.sourceType==="bank"||record.cardLast4===MAIN_DEBIT_CARD_LAST4)return primaryDebit?.id||nextAccounts[0]?.id||1;const key=record.cardLast4||record.accountHint;const known=nextAccounts.find(account=>record.cardLast4?account.last4===record.cardLast4||account.name.includes(record.cardLast4):account.name===record.accountHint);if(known)return known.id;if(record.cardLast4===CREDIT_CARD_LAST4){const genericCards=nextAccounts.filter(account=>account.type==="credit"&&!account.last4&&!/\d{4}/.test(account.name));if(genericCards.length){genericCards[0].last4=CREDIT_CARD_LAST4;genericCards[0].name=`Visa •••• ${CREDIT_CARD_LAST4}`;genericCards[0].linkedAccountId=primaryDebit?.id;return genericCards[0].id}}if(accountIds.has(key))return accountIds.get(key)!;const id=newEntityId()+nextAccounts.length;nextAccounts.push({id,name:record.cardLast4===CREDIT_CARD_LAST4?`Visa •••• ${CREDIT_CARD_LAST4}`:record.accountHint,type:"credit",balance:0,linkedAccountId:primaryDebit?.id,billingDay:15,last4:record.cardLast4});accountIds.set(key,id);return id}
    const created=createImportedTransactions(chosen,resolveAccount);const seriesIds=new Set(chosen.map(record=>record.installmentSeriesId).filter(Boolean));const normalized=normalizePersonalAccounts(nextAccounts,created);
    setAccounts(normalized.accounts);setTransactions(current=>[...normalized.entries,...current.filter(item=>!(item.posted===false&&item.installmentSeriesId&&seriesIds.has(item.installmentSeriesId)))]);setImported(created.length);setSelected(new Set());
  }
  const duplicateCount=records.filter(isExisting).length;const selectedTotal=records.filter(record=>selected.has(record.id)&&!isExisting(record)).reduce((sum,record)=>sum+record.amount,0);
  return <div className="modal-wrap bank-import-wrap" onMouseDown={onClose}><section className="quick-modal bank-import-modal" onMouseDown={event=>event.stopPropagation()}><div><span className="eyebrow">ФИНАНСЫ · ИМПОРТ ИЗ БАНКА</span><button type="button" onClick={onClose}>×</button></div><div className="bank-import-head"><div><h2>{imported===null?"Загрузите банковские выгрузки":"Импорт завершён"}</h2><p>{imported===null?"Можно выбрать CSV банка и несколько Excel-файлов карт одновременно. Файлы обрабатываются прямо в браузере.":imported?`${imported} операций добавлено и сохранено в вашей системе.`:"Новых операций нет — всё уже было импортировано."}</p></div>{imported!==null&&<span className="import-success">✓</span>}</div>{imported===null&&<><label className={`bank-dropzone ${loading?"loading":""}`}><input type="file" multiple accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={event=>void chooseFiles(event.target.files)}/><span>{loading?"◌":"⇩"}</span><strong>{loading?"Разбираю операции…":"Выбрать CSV или Excel"}</strong><small>Discount Bank · Visa/CAL · повторная загрузка безопасна</small></label>{error&&<div className="import-error">! {error}</div>}{fileSummary.length>0&&<><div className="import-file-summary">{fileSummary.map(file=><article key={file.name}><span>{file.format.includes("Excel")?"▤":"≡"}</span><div><strong>{file.format}</strong><small>{file.name}</small></div><b>{file.found}</b></article>)}</div><div className="import-kpis"><div><small>НАЙДЕНО</small><strong>{records.length}</strong></div><div><small>УЖЕ ЕСТЬ</small><strong>{duplicateCount+batchDuplicates}</strong></div><div><small>НЕ ЯВЛЯЮТСЯ ТРАТАМИ</small><strong>{ignored}</strong></div><div><small>ВЫБРАНО</small><strong>{selected.size} · {formatIls(selectedTotal)}</strong></div></div><div className="import-explanation"><span>◎</span><p><strong>Защита от двойного учёта включена.</strong> Списания Visa в банковском CSV не добавляются второй тратой. Покупки из двух Excel сопоставляются по карте, магазину, дате, исходной сумме и платежу.</p></div><div className="import-preview-head"><div><h3>Проверка перед импортом</h3><p>Русское название и категорию можно исправить прямо здесь.</p></div><button type="button" onClick={()=>setSelected(new Set(records.filter(record=>!isExisting(record)).map(record=>record.id)))}>Выбрать новые</button></div><div className="import-preview">{records.map(record=>{const duplicate=isExisting(record);return <article className={`${duplicate?"duplicate":""} ${selected.has(record.id)?"selected":""}`} key={record.id}><label><input type="checkbox" disabled={duplicate} checked={!duplicate&&selected.has(record.id)} onChange={()=>toggle(record.id)}/><span>✓</span></label><time>{prettyDate(record.date)}</time><div className="import-name"><input value={record.title} disabled={duplicate} onChange={event=>updateRecord(record.id,{title:event.target.value})}/><small>{record.originalTitle} · {record.accountHint}</small></div><select value={record.category} disabled={duplicate} onChange={event=>updateRecord(record.id,{category:event.target.value})}>{categories.map(category=><option value={category.name} key={category.id}>{category.icon} {category.name}</option>)}</select><b className={record.kind}>{record.kind==="income"?"+":"−"}{formatIls(record.amount)}</b><em>{duplicate?"уже импортировано":record.installmentCount?`${record.installmentIndex}/${record.installmentCount} платёж`:record.source}</em></article>})}</div><div className="import-footer"><div><small>БУДЕТ ДОБАВЛЕНО</small><strong>{selected.size} операций · {formatIls(selectedTotal)}</strong></div><button type="button" className="primary" disabled={!selected.size} onClick={commit}>Импортировать выбранное</button></div></>}</>}{imported!==null&&<div className="import-done"><div><span>✓</span><h3>{imported?"Расходы готовы":"Дубликаты пропущены"}</h3><p>{imported?"Названия сохранены на русском, исходные названия остались внутри записи для проверки. Новые карты добавлены автоматически без изменения их баланса.":"Можно закрыть окно — существующие операции остались без изменений."}</p></div><button type="button" className="primary" onClick={onClose}>Готово</button><button type="button" onClick={()=>{setImported(null);setRecords([]);setFileSummary([])}}>Импортировать другие файлы</button></div>}</section></div>
}
...
function FinanceCategoriesModal({categories,setCategories,onRename,onDelete,onClose}:{categories:FinanceCategory[];setCategories:React.Dispatch<React.SetStateAction<FinanceCategory[]>>;onRename?:(previousName:string,nextName:string)=>void;onDelete?:(name:string)=>void;onClose:()=>void}){
  const[drafts,setDrafts]=useState<FinanceCategory[]>(()=>categories.map(category=>({...category})));const[name,setName]=useState("");const[icon,setIcon]=useState("✨");const[error,setError]=useState("");
...
function CreateModal({kind,accounts,projects,goals,areas,categories,setCategories,onRenameCategory,onDeleteCategory,initialProjectId,initialHabit,initialTask,initialGoal,onClose,onCreate}:{kind:Exclude<ModalKind,null>;accounts:Account[];projects:Project[];goals:Goal[];areas:LifeArea[];categories:FinanceCategory[];setCategories:React.Dispatch<React.SetStateAction<FinanceCategory[]>>;onRenameCategory:(previousName:string,nextName:string)=>void;onDeleteCategory:(name:string)=>void;initialProjectId:number|null;initialHabit:Habit|null;initialTask:Task|null;initialGoal:Goal|null;onClose:()=>void;onCreate:(draft:Record<string,unknown>)=>void}){
  const initialProject=projects.find(p=>p.id===initialProjectId);const[area,setArea]=useState(initialGoal?.area||initialTask?.area||initialHabit?.area||initialProject?.area||areas[0]?.name||"Личное");const[cover,setCover]=useState(initialGoal?.cover||"linear-gradient(135deg,#2c2455,#7c4dff 58%,#d565ff)");const[goalPeriod,setGoalPeriod]=useState(initialGoal?.period||"МЕСЯЦ");const titles:Record<Exclude<ModalKind,null>,string>={task:initialTask?"Редактировать задачу":"Новая задача",project:"Новый проект",goal:initialGoal?"Редактировать цель":"Новая цель",habit:initialHabit?"Редактировать привычку":"Новая привычка",transaction:"Новая операция",account:"Новый счёт",transfer:"Перевод между счетами",health:"Метрика здоровья",event:"Новый план"};
  ...
  return <div className="modal-wrap" onMouseDown={onClose}><form className="quick-modal entity-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}><div><span className="eyebrow">NEXUS · СОЗДАНИЕ</span><button type="button" onClick={onClose}>×</button></div><h2>{titles[kind]}</h2>
    {!["transfer"].includes(kind)&&<Field label={kind==="health"?"Показатель":"Название"} name="title" placeholder="Введите название" defaultValue={initialGoal?.title||initialTask?.title||initialHabit?.name}/>} 
    ...
    <button className="primary modal-submit" type="submit">{initialHabit||initialTask||initialGoal?"Сохранить изменения":"Создать и сохранить"}</button></form></div>;
}
