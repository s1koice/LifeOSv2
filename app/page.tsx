...
export default function Home() {
  ...
  return <div className={`app-shell theme-${theme}`}>
    ...
    {authChecked&&authOpen&&<AuthPanel session={authSession} syncStatus={syncStatus} syncError={syncError} lastSyncedAt={lastSyncedAt} onSession={acceptAuthSession} onSignOut={handleSignOut} onSync={()=>void syncNow()} onBackup={downloadBackup} onClose={()=>{if(authSession)setAuthOpen(false)}}/>} 
...
function AuthPanel({session,syncStatus,syncError,lastSyncedAt,onSession,onSignOut,onSync,onBackup,onClose}:{session:PinSession|null;syncStatus:SyncStatus;syncError:string;lastSyncedAt:string;onSession:(session:PinSession)=>void;onSignOut:()=>void;onSync:()=>void;onBackup:()=>void;onClose:()=>void}){
