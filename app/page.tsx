...
export default function Home() {
  ...
  function content() {
    ...
    if (section === "Проекты") return <ProjectsPage projects={projects} setProjects={setProjects} tasks={tasks} setTasks={setTasks} lifeAreas={lifeAreas} setLifeAreas={setLifeAreas} inboxItems={inboxItems} setInboxItems={setInboxItems} resources={resources} setResources={setResources} selectedId={selectedProjectId} setSelectedId={setSelectedProjectId} onNew={() => setModalKind("project")} onNewTask={projectId => { setTaskProjectId(projectId); setModalKind("task"); }}/>;
...
function ProjectsPage({projects,setProjects,tasks,setTasks,lifeAreas,setLifeAreas,inboxItems,setInboxItems,resources,setResources,selectedId,setSelectedId,onNew,onNewTask}:{projects:Project[];setProjects:React.Dispatch<React.SetStateAction<Project[]>>;tasks:Task[];setTasks:React.Dispatch<React.SetStateAction<Task[]>>;lifeAreas:LifeArea[];setLifeAreas:React.Dispatch<React.SetStateAction<LifeArea[]>>;inboxItems:InboxItem[];setInboxItems:React.Dispatch<React.SetStateAction<InboxItem[]>>;resources:ResourceItem[];setResources:React.Dispatch<React.SetStateAction<ResourceItem[]>>;selectedId:number|null;setSelectedId:(id:number|null)=>void;onNew:()=>void;onNewTask:(projectId:number)=>void}){
  const[tab,setTab]=useState<"inbox"|"projects"|"areas"|"resources"|"archive">("inbox");
...
function PlanningPage({events,setEvents,tasks,setTasks,focuses,setFocuses,onNew,notify}:{events:CalendarEvent[];setEvents:React.Dispatch<React.SetStateAction<CalendarEvent[]>>;tasks:Task[];setTasks:React.Dispatch<React.SetStateAction<Task[]>>;focuses:Record<string,string>;setFocuses:React.Dispatch<React.SetStateAction<Record<string,string>>>;onNew:()=>void;notify:(s:string)=>void}){
  type ScheduleItem={key:string;id:number;type:"event"|"task";title:string;date:string;time:string;note:string;tone?:CalendarEvent["tone"];done?:boolean};
  ...
  const schedule:ScheduleItem[]=[...events.map(event=>({key:`event-${event.id}`,id:event.id,type:"event" as const,title:event.title,date:eventDate(event),time:event.time,note:event.note||"План",tone:event.tone})),...tasks.filter(task=>task.dueDate&&!task.archived).map(task=>({key:`task-${task.id}`,id:task.id,type:"task" as const,title:task.title,date:task.dueDate||"",time:/^\d{2}:\d{2}$/.test(task.time)?task.time:"08:00",note:`Задача · ${task.area}`,done:task.done}))];
  ...
  return <><PageTitle eyebrow="ВРЕМЯ" title="Планирование" text="Планы и задачи привязаны к настоящим датам и появляются только в своей неделе." action="Новый план" onAction={onNew}/><section className={`card planning-note ${focusSaved?"saved":""}`}><div><span className="eyebrow">ФОКУС НЕДЕЛИ</span><h3>{note||"Что должно стать главным результатом?"}</h3><small>{prettyDate(weekKey)} — {prettyDate(localDateKey(dates[6]))}</small></div><textarea value={note} onChange={event=>{setFocuses(current=>({...current,[weekKey]:event.target.value}));setSavedWeek("")}} placeholder="Например: закончить MVP и оставить пятницу без встреч"/><button type="button" disabled={!note.trim()} onClick={()=>{setSavedWeek(weekKey);notify("Фокус этой недели сохранён")}}>{focusSaved?"✓ Сохранено":"Сохранить фокус"}</button></section><div className="calendar-toolbar"><button type="button" onClick={()=>setWeekOffset(v=>v-1)} aria-label="Предыдущая неделя">←</button><strong>{prettyDate(localDateKey(dates[0]))} — {prettyDate(localDateKey(dates[6]))}</strong><button type="button" onClick={()=>setWeekOffset(v=>v+1)} aria-label="Следующая неделя">→</button><button type="button" onClick={()=>setWeekOffset(0)}>Сегодня</button><div className="calendar-legend"><span>● Планы</span><span>● Задачи</span></div></div><section className="week-calendar card"><div className="calendar-hours">{["08:00","10:00","12:00","14:00","16:00","18:00","20:00"].map(x=><span key={x}>{x}</span>)}</div>{dates.map((date,i)=>{const key=localDateKey(date);const dayItems=schedule.filter(item=>item.date===key).sort((a,b)=>a.time.localeCompare(b.time));return <div className={`calendar-day ${key===localDateKey(new Date())?"today":""}`} key={key}><strong>{dayNames[i]} {date.getDate()}</strong>{dayItems.map((item,index)=>{const hour=/^\d{2}:\d{2}$/.test(item.time)?Number(item.time.slice(0,2)):8;return <div className={`event ${item.type==="task"?"task-calendar-event":`${item.tone}-event`} ${item.done?"completed":""}`} style={{top:Math.max(54,54+(hour-8)*28+index*7),height:58}} key={item.key}>{item.type==="task"&&<button type="button" className="calendar-task-check" aria-label={item.done?"Вернуть задачу в работу":"Выполнить задачу"} onClick={()=>setTasks(current=>current.map(task=>task.id===item.id?{...task,done:!task.done}:task))}>{item.done?"✓":"○"}</button>}<span>{item.title}</span><small>{item.type==="task"?item.note:item.time}</small>{item.type==="event"&&<button type="button" aria-label="Удалить событие" onClick={()=>setEvents(current=>current.filter(event=>event.id!==item.id))}>×</button>}</div>})}</div>})}</section><section className="card upcoming-plans"><CardHead title="Ближайшие планы и задачи" subtitle={`${upcoming.length} впереди`} action="Добавить план" onClick={onNew}/>{upcoming.slice(0,10).map(item=><div className={item.type==="task"?"upcoming-task":""} key={item.key}><span>{new Date(`${item.date}T12:00`).toLocaleDateString("ru-RU",{day:"numeric",month:"short",weekday:"short"})}</span><strong>{item.time}</strong><div><b>{item.title}</b><small>{item.note}</small></div><button type="button" aria-label={item.type==="task"?"Выполнить задачу":"Удалить план"} onClick={()=>item.type==="task"?setTasks(current=>current.map(task=>task.id===item.id?{...task,done:true}:task)):setEvents(current=>current.filter(event=>event.id!==item.id))}>{item.type==="task"?"✓":"×"}</button></div>)}</section></>;
}
