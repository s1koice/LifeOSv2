...
type Transaction = { id: number; title: string; category: string; amount: number; kind: FinanceKind; date: string; accountId: number; toAccountId?: number; parentId?: number; installmentIndex?: number; installmentCount?: number; dueDate?: string; posted?: boolean; recurringId?: number; recurringPeriod?: string; importFingerprint?: string; importSource?: string; merchantOriginal?: string; importNote?: string; reconciled?: boolean };
...
function FinancePage({accounts,setAccounts,transactions,setTransactions,categories,setCategories,budgetLines,setBudgetLines,recurringExpenses,setRecurringExpenses,onRenameCategory,onDeleteCategory,onOperation,onTransfer,onAccount}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];setCategories:React.Dispatch<React.SetStateAction<FinanceCategory[]>>;budgetLines:BudgetLine[];setBudgetLines:React.Dispatch<React.SetStateAction<BudgetLine[]>>;recurringExpenses:RecurringExpense[];setRecurringExpenses:React.Dispatch<React.SetStateAction<RecurringExpense[]>>;onRenameCategory:(previousName:string,nextName:string)=>void;onDeleteCategory:(name:string)=>void;onOperation:()=>void;onTransfer:()=>void;onAccount:()=>void}){
  ...
  const selectedAccount=accounts.find(account=>account.id===selectedAccountId);const matchesSelectedAccount=(item:Transaction)=>selectedAccountId===null||item.accountId===selectedAccountId||item.toAccountId===selectedAccountId;const smallExpenses=monthTransactions.filter(item=>matchesSelectedAccount(item)&&item.kind==="expense"&&item.posted!==false&&item.amount<=10);const smallExpenseIds=new Set(smallExpenses.map(item=>item.id));const smallExpenseTotal=smallExpenses.reduce((sum,item)=>sum+item.amount,0);const journalTransactions=transactions.filter(item=>matchesSelectedAccount(item)&&(item.kind!=="installment"||item.posted)&&(showSmallExpenses||!smallExpenseIds.has(item.id)));
...
function FinanceCalendar({recurringExpenses,transactions,accounts}:{recurringExpenses:RecurringExpense[];transactions:Transaction[];accounts:Account[]}){
  ...
  return <section className="card finance-calendar"><div className="finance-calendar-head"><div><span className="eyebrow">ДЕНЕЖНЫЙ КАЛЕНДАРЬ</span><h3>Плановые расходы и платежи</h3><p>Календарь напоминает о датах, но ничего не списывает автоматически. Факт появляется из импортированной выписки.</p></div><div className="finance-calendar-summary"><span><small>ПОСТОЯННЫЕ</small><strong>{formatIls(recurringTotal)}</strong></span><span><small>ПЛАТЕЖИ КАРТ</small><strong>{formatIls(creditTotal)}</strong></span><span><small>ВСЕГО ПО ПЛАНУ</small><strong>{formatIls(recurringTotal+creditTotal)}</strong></span></div></div><div className="finance-calendar-toolbar"><button type="button" onClick={()=>move(-1)} aria-label="Предыдущий месяц">←</button><strong>{new Intl.DateTimeFormat("ru-RU",{month:"long",year:"numeric"}).format(month)}</strong><button type="button" onClick={()=>move(1)} aria-label="Следующий месяц">→</button><button type="button" onClick={()=>{const today=new Date();setMonth(new Date(today.getFullYear(),today.getMonth(),1,12))}}>Сегодня</button><div className="finance-calendar-legend"><span>● Постоянный</span><span>● Платёж карты</span><span>✓ Найден в выписке</span></div></div><div className="finance-calendar-scroll"><div className="finance-calendar-inner"><div className="finance-calendar-weekdays">{dayNames.map(day=><span key={day}>{day}</span>)}</div><div className="finance-calendar-grid">{dates.map(date=>{const key=localDateKey(date);const dayItems=items.filter(item=>item.date===key);return <div className={`finance-calendar-day ${date.getMonth()!==monthIndex?"outside":""} ${key===localDateKey(new Date())?"today":""}`} key={key}><strong>{date.getDate()}</strong><div>{dayItems.map(item=><article className={`finance-calendar-event ${item.kind} ${item.paid?"paid":""}`} title={`${item.title} · ${item.account}`} key={item.id}><span>{item.kind==="credit"?"◇":"↻"} {item.title}</span><b>{formatIls(item.amount)}</b><small>{item.paid?"✓ найдено в выписке":item.account}</small></article>)}</div></div>})}</div></div></div></section>;
}
...
function BankImportModal({accounts,setAccounts,transactions,setTransactions,categories,onClose}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];onClose:()=>void}){
  ...
  const existingFingerprints=useMemo(()=>new Set(transactions.map(transaction=>transaction.importFingerprint).filter(Boolean)),[transactions]);
  ...
  function isExisting(record:BankImportRecord){if(existingFingerprints.has(record.fingerprint))return true;return transactions.some(transaction=>!transaction.importFingerprint&&transaction.date===record.date&&Math.abs(transaction.amount-record.amount)<.01&&(transaction.merchantOriginal===record.originalTitle||transaction.title===record.title||Boolean(transaction.recurringId&&transaction.category===record.category)))}
  ...
  function commit(){const chosen=records.filter(record=>selected.has(record.id)&&!isExisting(record));if(!chosen.length){setImported(0);return}const nextAccounts=accounts.map(account=>({...account}));const primaryDebit=nextAccounts.find(account=>account.type==="debit")||nextAccounts[0];if(primaryDebit){primaryDebit.name="Основной счёт · Visa •••• 1078";primaryDebit.last4=MAIN_DEBIT_CARD_LAST4}const accountIds=new Map<string,number>();function resolveAccount(record:BankImportRecord){if(record.sourceType==="bank"||record.cardLast4===MAIN_DEBIT_CARD_LAST4)return primaryDebit?.id||nextAccounts[0]?.id||1;const key=record.cardLast4||record.accountHint;const known=nextAccounts.find(account=>record.cardLast4?account.last4===record.cardLast4||account.name.includes(record.cardLast4):account.name===record.accountHint);if(known)return known.id;if(record.cardLast4===CREDIT_CARD_LAST4){const genericCards=nextAccounts.filter(account=>account.type==="credit"&&!account.last4&&!/\d{4}/.test(account.name));if(genericCards.length){genericCards[0].last4=CREDIT_CARD_LAST4;genericCards[0].name=`Visa •••• ${CREDIT_CARD_LAST4}`;genericCards[0].linkedAccountId=primaryDebit?.id;return genericCards[0].id}}if(accountIds.has(key))return accountIds.get(key)!;const id=newEntityId()+nextAccounts.length;nextAccounts.push({id,name:record.cardLast4===CREDIT_CARD_LAST4?`Visa •••• ${CREDIT_CARD_LAST4}`:record.accountHint,type:"credit",balance:0,linkedAccountId:primaryDebit?.id,billingDay:15,last4:record.cardLast4});accountIds.set(key,id);return id}const created=chosen.map((record,index):Transaction=>({id:newEntityId()+index,title:record.title.trim()||"Импортированная операция",category:record.category,amount:record.amount,kind:record.kind,date:record.date,accountId:resolveAccount(record),posted:true,installmentIndex:record.installmentIndex,installmentCount:record.installmentCount,importFingerprint:record.fingerprint,importSource:record.source,merchantOriginal:record.originalTitle,importNote:record.note}));const normalized=normalizePersonalAccounts(nextAccounts,created);setAccounts(normalized.accounts);setTransactions(current=>[...normalized.entries,...current]);setImported(created.length);setSelected(new Set())}
