...
export default function Home() {
  ...
  const applyStoredState = useCallback((parsed: Record<string, unknown>) => {
    ...
    const financial = { accounts: uniqueEntityIds(Array.isArray(parsed.accounts) ? parsed.accounts as Account[] : seedAccounts), entries: uniqueEntityIds(Array.isArray(parsed.transactions) ? parsed.transactions as Transaction[] : seedTransactions) };
...
function Dashboard({userName,tasks,habits,projects,goals,lifeAreas,events,transactions,budgetLines,weeklyReviews,focusSessions,dailyCapacity,onDailyCapacity,taskCompletion,habitCompletion,today,onToggleTask,onToggleHabit,onStartFocus,navigate,order,setOrder,gamification,onGameCheckIn,onPlanWithAi}:{userName:string;tasks:Task[];habits:Habit[];projects:Project[];goals:Goal[];lifeAreas:LifeArea[];events:CalendarEvent[];transactions:Transaction[];budgetLines:BudgetLine[];weeklyReviews:ParaWeeklyReview[];focusSessions:FocusSession[];dailyCapacity:number;onDailyCapacity:(minutes:number)=>void;taskCompletion:number;habitCompletion:number;today:string;onToggleTask:(id:number)=>void;onToggleHabit:(id:number,date:string)=>void;onStartFocus:(id:number)=>void;navigate:(s:Section)=>void;order:DashboardBlockId[];setOrder:React.Dispatch<React.SetStateAction<DashboardBlockId[]>>;gamification:GamificationState;onGameCheckIn:(kind:"setback"|"recovery")=>void;onPlanWithAi:()=>void}) {
  ...
  const[editing,setEditing]=useState(false);const[dragging,setDragging]=useState<DashboardBlockId|null>(null);const[pointerDragging,setPointerDragging]=useState<DashboardBlockId|null>(null);const[goalArea,setGoalArea]=useState("");
  ...
  const blocks:Record<DashboardBlockId,React.ReactNode>={
    ...
    goals:<section className="card goals-card goal-ladder-card"><CardHead title="Главная линия целей" subtitle="Выберите сферу и двигайтесь снизу вверх: день → неделя → месяц → год" action="Настроить связи" onClick={()=>navigate("Цели")}/><div className="goal-area-switch">{goalAreas.map(area=>{const meta=lifeAreas.find(item=>item.name===area);return <button type="button" className={activeGoalArea===area?"active":""} key={area} onClick={()=>setGoalArea(area)}><span>{meta?.icon||"◎"}</span>{area}</button>})}{!goalAreas.length&&<button type="button" onClick={()=>navigate("Цели")}>＋ Создать цель дня</button>}</div><div className="goal-ladder"><div className="goal-ladder-note"><span>↑</span><div><strong>СНИЗУ ВВЕРХ</strong><small>{activeGoalArea}</small></div></div>{goalPeriodOrder.map((period,index)=>{const goal=activeGoalChain[period];const progress=goal?goalProgressValue(goal,projects,tasks,goals):0;const linkedTasks=goal?tasks.filter(task=>task.goalId===goal.id).length:0;const parent=goal?goals.find(item=>item.id===goal.parentGoalId):undefined;return <div className={`goal-ladder-level level-${period.toLowerCase()} ${period==="СЕГОДНЯ"?"current":""}`} key={period}>{index>0&&<span className="goal-connector">↑</span>}<button type="button" onClick={()=>navigate("Цели")}><div className="goal-level-icon">{period==="ГОД"?"✦":period==="МЕСЯЦ"?"◐":period==="НЕДЕЛЯ"?"▤":"✓"}</div><div className="goal-level-copy"><span>{period} · {goal?.date||"не задано"}</span><strong>{goal?.title||`Добавить цель · ${period.toLowerCase()}`}</strong><small>{parent?`Связано с: ${parent.title}`:period==="ГОД"?"Главное направление":"Связь не настроена"}</small></div><div className="goal-level-progress"><b>{progress}%</b><div><i style={{width:`${progress}%`}}/></div><small>{goal?.projectIds?.length||0} проектов · {linkedTasks} задач</small></div></button></div>})}</div></section>,
...
function BankImportModal({accounts,setAccounts,transactions,setTransactions,categories,onClose}:{accounts:Account[];setAccounts:React.Dispatch<React.SetStateAction<Account[]>>;transactions:Transaction[];setTransactions:React.Dispatch<React.SetStateAction<Transaction[]>>;categories:FinanceCategory[];onClose:()=>void}){
  ...
  function commit(){const chosen=records.filter(record=>selected.has(record.id)&&!isExisting(record));if(!chosen.length){setImported(0);return}const nextAccounts=accounts.map(account=>({...account}));const primaryDebit=nextAccounts.find(account=>account.type==="debit")||nextAccounts[0];const accountIds=new Map<string,number>();function resolveAccount(record:BankImportRecord){if(record.sourceType==="bank")return primaryDebit?.id||nextAccounts[0]?.id||1;const key=record.cardLast4||record.accountHint;const known=nextAccounts.find(account=>record.cardLast4?account.last4===record.cardLast4||account.name.includes(record.cardLast4):account.name===record.accountHint);if(known)return known.id;if(record.cardLast4){const genericCards=nextAccounts.filter(account=>account.type==="credit"&&!account.last4&&!/\d{4}/.test(account.name));if(genericCards.length===1){genericCards[0].last4=record.cardLast4;genericCards[0].name=record.accountHint;return genericCards[0].id}}if(accountIds.has(key))return accountIds.get(key)!;const id=newEntityId()+nextAccounts.length;nextAccounts.push({id,name:record.accountHint,type:"credit",balance:0,billingDay:15,last4:record.cardLast4});accountIds.set(key,id);return id}const created=chosen.map((record,index):Transaction=>({id:newEntityId()+index,title:record.title.trim()||"Импортированная операция",category:record.category,amount:record.amount,kind:record.kind,date:record.date,accountId:resolveAccount(record),posted:true,installmentIndex:record.installmentIndex,installmentCount:record.installmentCount,importFingerprint:record.fingerprint,importSource:record.source,merchantOriginal:record.originalTitle,importNote:record.note}));setAccounts(nextAccounts);setTransactions(current=>[...created,...current]);setImported(created.length);setSelected(new Set())}
