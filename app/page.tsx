...
type DashboardBlockId = "dayCenter" | "weekInsight" | "focus" | "tracker" | "today" | "compass" | "habits" | "goals" | "projects" | "reflection" | "game";
...
const defaultDashboardOrder: DashboardBlockId[] = ["dayCenter", "goals", "weekInsight", "focus", "game", "tracker", "today", "compass", "habits", "projects", "reflection"];
const seedGamification: GamificationState = { xp: 2840, events: [], activeDays: [], dailyScores: [] };
const gameBlockLabels: Record<DashboardBlockId, string> = { dayCenter: "Умный центр дня", weekInsight: "План и факт недели", focus: "Фокус дня", game: "Уровень и XP", tracker: "Трекер", today: "Задачи", compass: "Компас", habits: "Привычки", goals: "Цели", projects: "Проекты", reflection: "Разбор дня" };
...
export default function Home() {
  ...
  function handleCreate(kind: Exclude<ModalKind, null>, draft: Record<string, unknown>) {
    ...
    if (kind === "goal") {
      const fields = { period: String(draft.period || "МЕСЯЦ"), date: String(draft.date || "Сейчас"), title: String(draft.title), note: String(draft.note || "Новая цель"), progress: Math.max(0, Math.min(100, Number(draft.progress || editingGoal?.progress || 0))), area: String(draft.area || "Развитие"), parentGoalId: Number(draft.parentGoalId || 0) || undefined };
...
function Dashboard({userName,tasks,habits,projects,goals,lifeAreas,events,transactions,budgetLines,weeklyReviews,focusSessions,dailyCapacity,onDailyCapacity,taskCompletion,habitCompletion,today,onToggleTask,onToggleHabit,onStartFocus,navigate,order,setOrder,gamification,onGameCheckIn,onPlanWithAi}:{userName:string;tasks:Task[];habits:Habit[];projects:Project[];goals:Goal[];lifeAreas:LifeArea[];events:CalendarEvent[];transactions:Transaction[];budgetLines:BudgetLine[];weeklyReviews:ParaWeeklyReview[];focusSessions:FocusSession[];dailyCapacity:number;onDailyCapacity:(minutes:number)=>void;taskCompletion:number;habitCompletion:number;today:string;onToggleTask:(id:number)=>void;onToggleHabit:(id:number,date:string)=>void;onStartFocus:(id:number)=>void;navigate:(s:Section)=>void;order:DashboardBlockId[];setOrder:React.Dispatch<React.SetStateAction<DashboardBlockId[]>>;gamification:GamificationState;onGameCheckIn:(kind:"setback"|"recovery")=>void;onPlanWithAi:()=>void}) {
  ...
  const spans:Record<DashboardBlockId,"full"|"half">={dayCenter:"full",weekInsight:"full",focus:"full",game:"full",tracker:"full",today:"half",compass:"half",habits:"half",goals:"full",projects:"half",reflection:"half"};
