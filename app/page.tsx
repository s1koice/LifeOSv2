...
const defaultDashboardOrder: DashboardBlockId[] = ["dayCenter", "recommendations", "goals", "weekInsight", "focus", "game", "tracker", "today", "compass", "habits", "projects", "reflection"];
...
function Dashboard({userName,tasks,habits,projects,goals,lifeAreas,events,transactions,budgetLines,weeklyReviews,focusSessions,inboxItems,journalEntries,dailyCapacity,onDailyCapacity,taskCompletion,habitCompletion,today,onToggleTask,onToggleHabit,onStartFocus,navigate,order,setOrder,gamification,onGameCheckIn,onPlanWithAi}:{userName:string;tasks:Task[];habits:Habit[];projects:Project[];goals:Goal[];lifeAreas:LifeArea[];events:CalendarEvent[];transactions:Transaction[];budgetLines:BudgetLine[];weeklyReviews:ParaWeeklyReview[];focusSessions:FocusSession[];inboxItems:InboxItem[];journalEntries:JournalEntry[];dailyCapacity:number;onDailyCapacity:(minutes:number)=>void;taskCompletion:number;habitCompletion:number;today:string;onToggleTask:(id:number)=>void;onToggleHabit:(id:number,date:string)=>void;onStartFocus:(id:number)=>void;navigate:(s:Section)=>void;order:DashboardBlockId[];setOrder:React.Dispatch<React.SetStateAction<DashboardBlockId[]>>;gamification:GamificationState;onGameCheckIn:(kind:"setback"|"recovery")=>void;onPlanWithAi:()=>void}) {
  ...
  const spans:Record<DashboardBlockId,"full"|"half">={dayCenter:"full",recommendations:"full",weekInsight:"full",focus:"full",game:"full",tracker:"full",today:"half",compass:"half",habits:"half",goals:"full",projects:"half",reflection:"half"};
